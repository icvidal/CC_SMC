...
public:
	...
	virtual ~TwoCoinTurnstile() {}
	void Coin() {processEvent(Event_Coin, "Coin");}
	void Pass() {processEvent(Event_Pass, "Pass");}
	void Reset() {processEvent(Event_Reset, "Reset");}
	enum State {State_Alarming,State_FirstCoin,State_Locked,State_Unlocked};
	State state;
	enum Event {Event_Coin,Event_Pass,Event_Reset};
	virtual void queueEvent(Event event) {}
	virtual void dispatchEvent(Event event, const char *eventName) {}
...
default:
unexpected_transition("Invalid State", eventName);
break;
}
}
